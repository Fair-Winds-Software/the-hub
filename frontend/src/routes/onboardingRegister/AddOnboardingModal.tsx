// Authorized by HUB-1397 (E-CMP-WAVE4 S4) — Add Onboarding modal. Fields match the
// HUB-1384 schema (product_id / employee_name / employee_email / role / hire_date)
// rather than the story's aspirational names (employee_id / manager_id).
import { useEffect, useRef, useState, type FormEvent } from 'react';
import { apiClient } from '../../lib/api';
import { useToastStore } from '../../stores/toastStore';
import type { CreateOnboardingPayload, OnboardingRow } from './types';

export interface AddOnboardingModalProps {
  onClose: () => void;
  onCreated: (row: OnboardingRow) => void;
}

const initialDraft: CreateOnboardingPayload = {
  product_id: '',
  employee_name: '',
  employee_email: '',
  role: '',
...
export function AddOnboardingModal({ onClose, onCreated }: AddOnboardingModalProps): React.ReactElement {
  const [draft, setDraft] = useState<CreateOnboardingPayload>(initialDraft);
  const [submitting, setSubmitting] = useState(false);
  const [errors, setErrors] = useState<Partial<Record<keyof CreateOnboardingPayload, string>>>({});
  ...
  const set = <K extends keyof CreateOnboardingPayload>(k: K, v: string): void => {
    setDraft((d) => ({ ...d, [k]: v }));
  };
  ...
  const validate = (): boolean => {
    ...
    if (!draft.hire_date.trim()) e.hire_date = 'Required';
    setErrors(e);
  ...
  const onSubmit = async (evt: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setSubmitting(true);
    try {
      const row = await apiClient.post<OnboardingRow>('/api/v1/admin/grc/onboarding', {
      ...
      addToast({ variant: 'success', message: 'Onboarding record created.' });
      onCreated(row);
    } catch (err) {
      addToast({
        variant: 'error',
        message: err instanceof Error ? err.message : 'Failed to add onboarding record.',
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="add-onb-heading"
      data-testid="add-onb-modal"
    ...
    >
      <div className="w-full max-w-lg rounded-md bg-sailcloth p-4 shadow-lg">
        <h2 id="add-onb-heading" className="mb-3 font-heading text-lg text-primary-navy">
        ...
        <form onSubmit={(e) => void onSubmit(e)} className="flex flex-col gap-3">
          <label className="flex flex-col gap-1 text-sm font-body text-deep-charcoal/80">
            ...
            <input
              ref={firstFieldRef}
              data-testid="add-onb-product-id"
              type="text"
          ...
          <label className="flex flex-col gap-1 text-sm font-body text-deep-charcoal/80">
            Employee name
            <input
              data-testid="add-onb-employee-name"
              type="text"
          ...
          <label className="flex flex-col gap-1 text-sm font-body text-deep-charcoal/80">
            Employee email
            <input
              data-testid="add-onb-employee-email"
              type="email"
          ...
          <label className="flex flex-col gap-1 text-sm font-body text-deep-charcoal/80">
            Hire date
            <input
              data-testid="add-onb-hire-date"
              type="date"
              value={draft.hire_date}
              onChange={(e) => set('hire_date', e.target.value)}
              aria-required="true"
              aria-invalid={errors.hire_date ? true : undefined}
              className={fieldClass}
            />
            {errors.hire_date && <span className="text-xs text-error-crimson">{errors.hire_date}</span>}
          </label>
          <div className="mt-2 flex justify-end gap-2">
            <button
              type="button"
              data-testid="add-onb-cancel"
              onClick={onClose}
